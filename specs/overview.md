...
3. Each entry shows a human-readable label and has the full path as its id
...
- **Directory scanning**: `wezterm.read_dir` and `wezterm.glob` (no shell-outs
  for basic scanning; `wezterm.run_child_process` with `find` for `.git`
  detection in deep trees)
